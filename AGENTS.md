...
- `src/lib/voice-demo-scripts.json` (hero demo card) and `src/lib/industry-demo-scripts.json` (industries section) are the source of truth for their respective demo lines (slug, label, script text) — kept as two separate files specifically so adding an industry demo doesn't also add an entry to the hero's list. `voice-bots-content.ts` / `voice-bots-industries.ts` import their own file and derive each demo's `audio` path as `/audio/voice-demos/<slug>.mp3` — those files don't exist until generated.
- `npm run generate:voice-demos` (→ `scripts/generate-voice-demos.mjs`) reads both JSON files, calls Deepgram's Aura TTS API (`POST /v1/speak`) once per line, and writes the mp3s into `public/audio/voice-demos/`. Skips any slug whose mp3 already exists (set `FORCE=1` to redo it) so re-runs after adding one new line don't re-spend API credits regenerating everything. Requires `DEEPGRAM_API_KEY` — copy `.env.example` to `.env` (gitignored) and run via `node --env-file=.env scripts/generate-voice-demos.mjs`, or pass the key inline for a one-off run. Never commit a real key, and never paste one into chat — the `.env` file is the only place it should live.
- `voice-demo-player.tsx` (hero) and `industry-demo-button.tsx` (industries section) are small Client Components (play/pause via a plain `<audio>` element) — the only interactive pieces on the page; `voice-bots-hero.tsx` and `voice-bots-industries.tsx` themselves stay Server Components.
- To add another demo, add an entry to the relevant JSON file and re-run the generate script — everything else derives from it automatically. Adding a new *industry* also means adding an icon to the `icons` map in `voice-bots-industries.tsx` (slug → lucide icon).
