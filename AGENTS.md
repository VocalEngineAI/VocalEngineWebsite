...
- `voice-bots-integrations.tsx` (the orbit diagram on `/solutions/voice-bots`) deliberately uses generic category icons (Calendar, CRM & Sales, Email, Comms & Messaging) instead of real brand logos, framed as "runs on n8n, so anything n8n supports is possible" rather than naming specific tools — avoids reproducing trademarked logos (unlike the reference screenshot in `concept/`, which does show real logos) while still making a defensible breadth claim. Keep that framing rather than swapping in actual logos.
